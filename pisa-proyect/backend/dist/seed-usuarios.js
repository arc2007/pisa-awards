    ...
    `;
        db_1.db.serialize(() => {
            ...
            categorias.forEach((c) => {
                stmt.run([c.numero, c.nombre, c.descripcion || null, c.es_videos ? 1 : 0], (err) => {
                    if (err)
    ...
    `;
        ...
        db_1.db.serialize(() => {
            nominaciones.forEach((n) => {
                db_1.db.run(sqlInsertNom, [n.categoriaNumero, n.descripcion, n.video_url ?? null], (errNom) => {
                    if (errNom) {
                        console.error("Error insertando nominación", n, errNom.message);
                        return doneNominacion();
                    }
                    db_1.db.get(sqlGetNomId, [n.categoriaNumero, n.descripcion], (errGet, rowNom) => {
                        if (errGet || !rowNom) {
                            console.error("Error obteniendo id de nominación", n, errGet?.message ?? "No encontrada");
                            return doneNominacion();
                        }
                        const nominacionId = rowNom.id;
                        console.log("Nominación OK:", `cat ${n.categoriaNumero} -> ${n.descripcion} (id=${nominacionId})`);
                        ...
                        n.usernames.forEach((username) => {
                            db_1.db.get(sqlUsuarioId, [username], (errUser, rowUser) => {
                                if (errUser) {
                                ...
                                db_1.db.run(sqlInsertNomUsuario, [nominacionId, rowUser.id], (errLink) => {
                                    if (errLink) {
                                        console.error("Error insertando nominacion_usuarios", { nominacionId, username }, errLink.message);
                                    }
                                    else {
